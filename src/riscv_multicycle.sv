...
module riscv_multicycle import riscv_pkg::*; #(
    ...
    // SENİN MÜKEMMEL ÇALIŞAN DECODER'IN
    decoder u_decoder (
    ...
    // YENİ EKLENEN: TAZE VERİYİ HAVADA YAKALAMA (FORWARDING)
    logic [31:0] fwd_rs1_data, fwd_rs2_data;
    assign fwd_rs1_data = (wb_valid && wb_reg_we && (wb_rd_addr != 5'b0) && (wb_rd_addr == rs1_addr)) ? wb_data : rs1_data;
    assign fwd_rs2_data = (wb_valid && wb_reg_we && (wb_rd_addr != 5'b0) && (wb_rd_addr == rs2_addr)) ? wb_data : rs2_data;
    ...
    always_ff @(posedge clk_i) begin
        if (!rstn_i || flush || stall) begin 
            ...
            ex_pc <= dec_pc; 
            ex_imm <= imm; 
            ex_rs1_data <= fwd_rs1_data; // Bayat veri yerine taze veri gidiyor!
            ex_rs2_data <= fwd_rs2_data; // Bayat veri yerine taze veri gidiyor!
    ...
    logic [XLEN-1:0] mem_pc, mem_alu_res, mem_rs2_data, mem_id_reg, mem_instr_reg, mem_imm_reg;
    ...
    always_ff @(posedge clk_i) begin
        if (!rstn_i) begin
            ...
            mem_pc <= ex_pc; 
            mem_alu_res <= alu_res; // Kafa karıştıran ex_result kaldırıldı, direkt ALU sonucu.
    ...
    logic [XLEN-1:0] wb_pc, wb_alu_res, wb_mem_data, wb_id, wb_instr, wb_imm_reg;
    ...
    always_comb begin
        case (wb_sel_final)
            ...
            2'b11: wb_data = wb_imm_reg; // Senin decoderin LUI için 11 üretiyorsa, değer tam burada yakalanıyor.
    ...
    // DİKKAT: Artık Writeback (WB) aşamasındaki çakışmalarda sistemi durdurmuyoruz (Stall yapmıyoruz).
    // Çünkü yukarıda yazdığımız "fwd_rs1_data" mantığı sayesinde veri havada yakalanıyor!
    logic hazard_ex, hazard_mem;
    ...
    assign stall = dec_valid && (hazard_ex || hazard_mem) && !flush;
