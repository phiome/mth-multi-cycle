...
module riscv_multicycle import riscv_pkg::*; #(
    ...
    // SENİN ESKİ VE KUSURSUZ DECODERİN BURADA ÇALIŞIYOR
    decoder u_decoder (
    ...
    always_ff @(posedge clk_i) begin
        if (!rstn_i || flush || stall) begin 
            ...
            ex_pc <= dec_pc; 
            ex_imm <= imm; // IMM DEĞERİ ARTIK BORU HATTINDA TAŞINIYOR
            ex_rs1_data <= rs1_data; ex_rs2_data <= rs2_data;
    ...
    always_comb begin
        branch_taken = 1'b0;
        if (ex_branch) begin
            // SENİN CUSTOM ISA'NDAKİ BRANCH KODLARI
            case (ex_branch_type)
    ...
    always_ff @(posedge clk_i) begin
        if (!rstn_i) begin
            ...
            mem_pc <= ex_pc; mem_alu_res <= alu_res; 
            mem_rs2_data <= ex_rs2_data; 
            mem_imm_reg <= ex_imm; // IMM DEĞERİ TAŞINMAYA DEVAM EDİYOR
    ...
    always_ff @(posedge clk_i) begin
        if (!rstn_i) begin
            ...
            wb_imm_reg <= mem_imm_reg; // IMM DEĞERİ SON AŞAMAYA ULAŞTI
    ...
    always_comb begin
        case (wb_sel_final)
            ...
            2'b11: wb_data = wb_imm_reg; // SENİN ESKİ DECODERİN İÇİN IMM BURADAN ALINACAK
